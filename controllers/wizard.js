...
//      console.log("connecting to mailserver: " + process.env.SMTP_SERVER + ": port " + process.env.SMTP_PORT)
      const mailOptions = {
        ...
        to: req.params.id,
        subject: 'MEAN DEMO: Password Reset Request', // Subject line
        html: '<p>A request has been made to reset your password. If you made this request, click this <a href="' + process.env.RETURN_EMAIL_SERVER + '/resetpwd/' + verificationCode +'"> Verification Link </a> to reset your password.</p><p> Otherwise just ignore it and log in at your convenience.</p>'
      };
    ...
    wizard
      ...
      .then(result => {
        const mailOptions = {
          from: 'validate@pixieproductions.com',
          to: req.body.email,
          subject: 'MEANDEMO: Verify your email', // Subject line
          html: '<p>We need to verify your email: Click this <a href="' + process.env.RETURN_EMAIL_SERVER + '/validate/' + verificationCode +'">Verification Link</a> to validate your email and your account.</p>'
        };
        transporter.sendMail(mailOptions, function (err, info) {
          if(err)
            console.log("Error: " + err)
